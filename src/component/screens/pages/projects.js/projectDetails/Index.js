...
import {  createPartysProject, createProjects, getCoProducerByProducerId, getInvestorbyProducerId, getInvestorsByProjectId, getProjectByProducerId, getProjectsByID, updateProjects } from "../../../../redux/Action";
...
//   View Investor Docuents end
  ...
  useEffect(() => {
      if (investorsByProjectId?.data) {
        const mappedData = investorsByProjectId.data.map((item, index) => ({
          no: index + 1,
          investorname: `${item.firstname}${item.lastname}`,
          finalAmount: item.zipcode,
          status:"active",
          action: (
